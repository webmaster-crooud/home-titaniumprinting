...
export const DefaultButton: React.FC<propsDefaultButton> = ({ title, link, icon, className, onClick, onMouseEnter, onMouseLeave }) => {
    const defaultClass =
    ...
    return onMouseEnter || onClick || onMouseLeave ? (
        <button onClick={onClick} className={`${defaultClass} ${className}`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
            {icon}
